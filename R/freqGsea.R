...
freqGsea <- function(pathways, sep="_", bias=NULL, clean=T, cleanII=T, removeIncluding="", removeThis=""){
  ...
  if(clean){
    discard <- c("the","to","from","with","an","and","or","of","off","on","by","via","in","kegg","go","reactome","pid","wp","biocarta","unknown")
    discard <- c(discard,removeIncluding)
    discard <- c(paste0("^",discard,"-"),paste0("-",discard,"-"),paste0("-",discard,"$"),paste0("^",discard,"$"))
  }
  discard2 <- ""
  if(cleanII) discard2 <- c("a","activation","mediated","events","event","cell","cellular","activity","regulation","positive","negative","process","system","involved","containing","like","alpha","beta","gamma","delta",".","ii","iii","iv","v","vi","vii","viii","ix","up","dn","down")
